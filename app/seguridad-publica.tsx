...
type ProcessualProfile = "favorable" | "riesgo";
...
const processualInput = (flagrante: boolean | undefined, profile: ProcessualProfile | undefined): ProcessualInput => flagrante === true ? { flagrante: true } : { flagrante: false, plenamenteIdentificado: profile !== "riesgo", localizable: profile !== "riesgo", riesgoIncomparecencia: profile === "riesgo" };
...
function IncidentDecision({ concept, facts, setFacts, onNavigate }: { concept: PublicConcept; facts: PublicSafetyFacts; setFacts: Dispatch<SetStateAction<PublicSafetyFacts>>; onNavigate: (id: string) => void }) {
  ...
  const needsProcessualFacts = hasProcessualOutput && !fixedMinorOffence;
  const showResolvedProcessual = fixedMinorOffence || hasProcessualAnswer(flagrante, profile);
  const showComplaintQuestion = facts.relacion === "vg" && hasProcessualOutput;
  ...
  return <>
    ...
    {showComplaintQuestion && <><ToggleQuestion legend="¿La víctima desea formular denuncia? La actuación continuará aunque no lo haga." value={facts.noDeseaDenunciar === undefined ? undefined : !facts.noDeseaDenunciar} onChange={(value) => set("noDeseaDenunciar", !value)} />
      <div className="sp-question-help">{sexualContext ? "La respuesta no paraliza la protección ni las primeras diligencias. Para proceder por el delito sexual: denuncia de la víctima o querella del Ministerio Fiscal." : "La denuncia de la víctima no es necesaria. Informa de ello y continúa la actuación de oficio."}</div>
    </>}
...
function ProcessualControls({ flagrante, setFlagrante, profile, setProfile }: { flagrante: boolean | undefined; setFlagrante: (value: boolean) => void; profile: ProcessualProfile | undefined; setProfile: (value: ProcessualProfile | undefined) => void }) { const chooseFlagrancy = (value: boolean) => { setFlagrante(value); setProfile(undefined); }; return <fieldset className="sp-choice sp-processual-question"><legend>Presupuesto legal para la detención</legend><strong>¿La persona ha sido sorprendida durante la comisión del hecho o inmediatamente después?</strong><label><input type="radio" checked={flagrante === true} onChange={() => chooseFlagrancy(true)} /> Sí</label><label><input type="radio" checked={flagrante === false} onChange={() => chooseFlagrancy(false)} /> No</label>{flagrante === false && <div className="sp-nested-question"><strong>¿Está plenamente identificada y localizable, sin indicadores objetivos de riesgo de incomparecencia?</strong><label><input type="radio" checked={profile === "favorable"} onChange={() => setProfile("favorable")} /> Sí</label><label><input type="radio" checked={profile === "riesgo"} onChange={() => setProfile("riesgo")} /> No</label></div>}</fieldset> }
function AuthorSituation({ decision }: { decision: { situacion?: "DETENIDO" | "INVESTIGADO NO DETENIDO" | "DILIGENCIAS DE PREVENCIÓN"; detencion?: "SÍ" | "NO"; escenarioProcesal?: string; fundamentoDetencion?: string } }) { if (!decision.situacion || !decision.detencion) return null; const output = decision.situacion === "DETENIDO" ? "DETENER" : decision.situacion === "INVESTIGADO NO DETENIDO" ? "INVESTIGADO NO DETENIDO" : "DILIGENCIAS A PREVENCIÓN"; const tone = decision.situacion === "DETENIDO" ? "detain" : decision.situacion === "INVESTIGADO NO DETENIDO" ? "investigate" : "prevent"; return <section className="sp-section sp-processual-output"><h3>SITUACIÓN PROCESAL</h3><div className={`sp-decision-banner ${tone}`}><span aria-hidden="true" /><strong>{output}</strong><small>Detención: {decision.detencion === "SÍ" ? "sí" : "no"}</small></div>{decision.escenarioProcesal === "DELITO LEVE" && <p className="sp-status-note">Regla general: no detener. Excepción: art. 495 LECrim.</p>}</section> }
function Continuation({ decision, additionalItems = [] }: { decision: { situacion?: "DETENIDO" | "INVESTIGADO NO DETENIDO" | "DILIGENCIAS DE PREVENCIÓN" }; additionalItems?: string[] }) { if (!decision.situacion) return null; const next = decision.situacion === "DETENIDO" ? ["Informar de derechos y garantías al detenido.", "Solicitar presencia de CNP en el lugar.", "Entregar persona y actuaciones a CNP."] : decision.situacion === "INVESTIGADO NO DETENIDO" ? ["Informar de hechos y derechos.", "Coordinar con CNP la continuación y citación.", "Comparecer posteriormente cuando corresponda."] : ["Practicar diligencias a prevención.", "Comunicar las actuaciones a CNP."]; return <OperationalSection title="CONTINUACIÓN OPERATIVA" items={uniqueItems([...additionalItems, ...next])} /> }
