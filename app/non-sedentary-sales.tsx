...
export function VentaNoSedentariaCaseSheet({ item }: { item: OperationalCase }) {
  ...
  return <article className="vns-sheet"><div className="sheettitle"><div><span className="kicker">FICHA OPERATIVA</span><h2>{item.titulo}</h2></div></div>
    ...
    <section><h3>RESULTADO</h3>{ready ? chosen?.sinInfraccion ? <p>{result}</p> : <><p><strong>{classification}</strong> · {item.norma} · art. {article}</p><p>{result}</p><p><strong>Régimen sancionador previsto:</strong> {range(classification)}</p><small>La cuantía concreta corresponde al órgano sancionador.</small></> : <p>Completa los datos del supuesto para mostrar el resultado aplicable.</p>}</section>
