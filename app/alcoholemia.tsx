...
export function AlcoholemiaView({ onBack }: { onBack: () => void }) {
  ...
  return <section className="alcohol-view">
    ...
    <section className="alcohol-card calculator-card">
      ...
      <div className="calculator-layout">
        ...
        <div className={`calculation-result result-${outcome.tono}`}>
          ...
          {calculation && !negative && <div className="calculation-metrics"><Metric label="Lectura" value={formatMg(calculation.lectura_exacta)} /><Metric label="EMP aplicado" value={`${formatMg(calculation.emp_exacto)} mg/L`} /><Metric label="Tasa exacta corregida" value={`${formatMg(calculation.valor_corregido_exact)} mg/L`} /></div>}
    ...
    <div className="alcohol-bottom-grid">
      <section className="alcohol-card practice-card"><div className="alcohol-card-heading"><span className="alcohol-symbol tone-blue">▤</span><div><h3>5. Actuación, pruebas y medidas</h3><p>Recordatorio breve para la intervención</p></div></div><div className="practice-list"><InfoBlock title="Pruebas" items={[alcoholemia.practica_pruebas.segunda_prueba, alcoholemia.practica_pruebas.intervalo]} /><InfoBlock title="Derechos" items={alcoholemia.practica_pruebas.derechos} /><InfoBlock title="Vehículo · art. 25 RGC" items={alcoholemia.medidas_vehiculo.especifica_rgc_25} /></div><p className="cross-reference">La medida sobre el vehículo se documenta junto con la intervención y se rige por las reglas comunes vigentes.</p></section>
      <details className="alcohol-card sources-card"><summary className="alcohol-card-heading sources-summary"><span className="alcohol-symbol tone-indigo">ⓘ</span><span><strong>6. Fuentes jurídicas</strong><small>Referencias validadas del paquete operativo</small></span></summary><div className="source-list">{alcoholemia.fuentes_v3.map((source) => <article key={source.id}><strong>{source.nombre}</strong>{source.preceptos.length > 0 && <span>{source.preceptos.join(" · ")}</span>}<p>{source.uso}</p></article>)}</div></details>
    </div>
